const autoCompleteJS3 = new autoComplete({
  ...
  data: {
    src: async () => {
      try {
        document.getElementById("search-index").setAttribute("placeholder", "Loading...");
        // Fetch External Data Source
        const source = await fetch("./db/generic.json");
        const data = await source.json();
        // Post Loading placeholder text
        document.getElementById("search-index").setAttribute("placeholder", autoCompleteJS3.placeHolder);
  ...
  events: {
    input: {
      ...
      selection(event) {
        ...
        // Prepare User's Selected Values
        ...
        console.log(feedback.selection.value.Symbol);
