...
//         // document.getElementById("autoComplete2").setAttribute("placeholder", "Loading...");
...
//         //document.getElementById("autoComplete2").setAttribute("placeholder", autoCompleteJS.placeHolder);
...
//         OpenScripDetailsModal(feedback.selection.value.symbol, feedback.selection.value.name);
//       },
//     },
//   },
// });
...
const autoCompleteJS2 = new autoComplete({
  ...
  data: {
    src: async () => {
      try {
        ...
        console.log("autoCompleteJS2");

        document.getElementById("symbol").setAttribute("placeholder", "Loading...");
        // Fetch External Data Source
        const source = await fetch("./db/generic.json");
        const data = await source.json();
        // Post Loading placeholder text
        document.getElementById("symbol").setAttribute("placeholder", autoCompleteJS2.placeHolder);
        //document.querySelector(".autoComplete").setAttribute("placeholder", autoCompleteJS.placeHolder);
        // Returns Fetched data
        return data;
      } catch (error) {
  ...
  events: {
    input: {
      focus() {
        if (autoCompleteJS2.input.value.length) autoCompleteJS2.start();
      },
      selection(event) {
        const feedback = event.detail;
        console.log(feedback);
        ...
        document.querySelector(".selection").innerHTML = selection;
        // Replace Input value with the selected value
        rsymbol = feedback.selection.value.Symbol;
        rcompName = feedback.selection.value.Company;
...
//   autoCompleteJS2.input.addEventListener(eventType, () => {
//     // Blur page elements
//     if (eventType === "blur") {
//       action("dim");
